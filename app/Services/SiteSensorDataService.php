...
class SiteSensorDataService
{
    public function fetch(Request $request, int $id, bool $jsonResponse = true)
    {
        ...
        if ($timeframeColumn === 'minutes') {
            $sensorData = SensorData::with('data_file.site')
                ->whereHas('data_file.site', function ($query) use ($id) {
                    $query->where('id', $id);
                })
            ...
            $sensorData = SensorDataWindowedSite::where('site_id', $id)
        ...
        $formattedData = $sensorData->map(function ($data) use ($timerange) {
            ...
            return [
                'timestamp' => $timerange === '1h' ? ($data->timestamp ?? $data->window_end) : $data->window_end->setTimezone('UTC')->format($timeFormat),
